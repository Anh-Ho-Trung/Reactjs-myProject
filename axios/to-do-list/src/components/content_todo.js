...
export class Content_todo extends React.Component {
  ...
  // get  data
  componentDidMount() {
    axios.get(`http://5e05ffa88983960014ebed22.mockapi.io/todos`).then(res => {
  ...
  addTaskClick = (e, name) => {
    axios
      .post("http://5e05ffa88983960014ebed22.mockapi.io/todos", {
        name: name
      })
      .then(response => {
  ...
  deleteTaskClick = e => {
    axios
      .delete(
        `http://5e05ffa88983960014ebed22.mockapi.io/todos/` +
          e.target.dataset.idTask
      )
      .then(res => {
        console.log(res);
        if (res.status === 200) {
          let indexRemove = this.state.todos.findIndex(
            todos => todos.id === res.data.id
          );
  ...
  editTaskClick = e => {
    let indexEdit = this.state.todos.findIndex(
      todos => todos.id === e.target.dataset.idTask
    );
